...
namespace BusinessLogic.Services.Statistics
{
    public class GameStatisticService : IGameStatisticService
    {
        ...
        public List<string> TopDevelopers(string userID = "", int numToTake = 0)
            => string.IsNullOrWhiteSpace(userID)
        ...
        public List<string> MostCompleted(string userID = "", int numToTake = 0)
            => string.IsNullOrWhiteSpace(userID)
                ? _games.OrderByDescending(x => x.TimesCompleted).Select(y => y.Title).Take(numToTake > 0 ? numToTake : _games.Count).ToList()
