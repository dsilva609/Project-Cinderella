...
namespace BusinessLogic.Services.Statistics
{
    public class GameStatisticService : IGameStatisticService
    {
        ...
        public int NumRatedEC(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _games.Count(x => x.Rating == GameRatingEnum.EC)
                : _games.Count(x => x.UserID == userID && x.Rating == GameRatingEnum.EC);

        public int NumRatedE(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _games.Count(x => x.Rating == GameRatingEnum.E)
                : _games.Count(x => x.UserID == userID && x.Rating == GameRatingEnum.E);

        public int NumRatedE10(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _games.Count(x => x.Rating == GameRatingEnum.E10)
                : _games.Count(x => x.UserID == userID && x.Rating == GameRatingEnum.E10);

        public int NumRatedT(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _games.Count(x => x.Rating == GameRatingEnum.T)
                : _games.Count(x => x.UserID == userID && x.Rating == GameRatingEnum.T);

        public int NumRatedM(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _games.Count(x => x.Rating == GameRatingEnum.M)
                : _games.Count(x => x.UserID == userID && x.Rating == GameRatingEnum.M);

        public int NumRatedA(string userID = "")
            => string.IsNullOrWhiteSpace(userID)
                ? _games.Count(x => x.Rating == GameRatingEnum.A)
                : _games.Count(x => x.UserID == userID && x.Rating == GameRatingEnum.A);
