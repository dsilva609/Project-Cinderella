...
namespace UI.Controllers
{
	public partial class WishController : ProjectCinderellaControllerBase
	{
		...
		public virtual ActionResult Index(string wishQuery, string filter, int? page)
		{
			...
			{
				ViewTitle = "Index",
				AlbumWishes = wishes?.Where(x => x.ItemType == ItemType.Album).ToPagedList(page ?? 1, NUM_WISHES_TO_GET),
				BookWishes = wishes?.Where(x => x.ItemType == ItemType.Book).ToPagedList(page ?? 1, NUM_WISHES_TO_GET),
				MovieWishes = wishes?.Where(x => x.ItemType == ItemType.Movie).ToPagedList(page ?? 1, NUM_WISHES_TO_GET),
				GameWishes = wishes?.Where(x => x.ItemType == ItemType.Game).ToPagedList(page ?? 1, NUM_WISHES_TO_GET),
