...
namespace UI.Controllers
{
    public partial class AlbumController : ProjectCinderellaControllerBase
    {
        ...
        [HttpGet]
        public virtual ActionResult Index(string query, int? page)
        {
            //TODO: needs cleanup
            ViewBag.Query = query;
            ViewBag.PageNum = page ?? 1;
            var viewModel = new AlbumViewModel();
            viewModel.ViewTitle = "Index";
            var albums = _service.GetAll(User.Identity.GetUserId(), query);
            viewModel.PageSize = NUM_ALBUMS_TO_GET;
            viewModel.Albums = albums.ToPagedList(page ?? 1, viewModel.PageSize);
            viewModel.TotalItems = viewModel.Albums.Count;
